...
import { VectorCache } from './rag/vectorCache.js';
...
const MB = 1024 * 1024;
const VECTOR_CACHE_TOP_K = 12;
const VECTOR_CACHE_MIN_SIM = 0.18;
const VECTOR_CACHE_BATCH_SIZE = 128;
const VECTOR_CACHE_WINDOW = 50;
const VECTOR_CACHE_MISS_THRESHOLD = 0.6;
const VECTOR_CACHE_PRIME_LIMIT = 500;
const VECTOR_CACHE_PRIME_COOLDOWN = 60000;
const DRAWER_RENDER_INTERVAL_MS = 500;
const vectorCacheState = {
  supported: typeof indexedDB !== 'undefined',
  ready: false,
  hits: 0,
  misses: 0,
  lastSize: { entries: 0, bytes: 0 },
  status: 'Inactive',
  missWindow: [],
  lastPrime: 0,
  primeRunning: false
};
let vectorCacheSizeTimer = null;
let vectorCacheIngestTimer = null;
const vectorCacheQueue = [];
const chunkUsage = new Map();
const chunkRegistry = new Map();
let floatingMemoryRenderTimer = null;
let floatingMemoryRenderPending = false;
...
function trackMemoryEntry(entry) {
  ...
  registerChunk(normalized, entry.source || 'memory');
  if (normalized.agentId && normalized.agentId !== humanAgentId) {
    queueVectorCacheIngest(normalized, [normalized.agentId], 'memory');
  }
  maintainMemoryBudget();
  void persistFloatingMemory();
  updateMemoryStats();
  requestFloatingMemoryRender(false);
}

function removeMemoryEntry(id) {
  const index = floatingMemory.findIndex((entry) => entry.id === id);
  if (index === -1) return;
  const [removed] = floatingMemory.splice(index, 1);
  if (removed?.id) {
    chunkRegistry.delete(removed.id);
  }
  void persistFloatingMemory();
  updateMemoryStats();
  requestFloatingMemoryRender(true);
}
...
function renderFloatingMemoryList() {
...
function requestFloatingMemoryRender(immediate = false) {
  if (!elements.floatingMemoryList) return;
  if (immediate) {
    if (floatingMemoryRenderTimer) {
      clearTimeout(floatingMemoryRenderTimer);
      floatingMemoryRenderTimer = null;
    }
    floatingMemoryRenderPending = false;
    renderFloatingMemoryList();
    return;
  }
  if (floatingMemoryRenderPending) {
    return;
  }
  floatingMemoryRenderPending = true;
  floatingMemoryRenderTimer = setTimeout(() => {
    floatingMemoryRenderPending = false;
    floatingMemoryRenderTimer = null;
    renderFloatingMemoryList();
  }, DRAWER_RENDER_INTERVAL_MS);
}
...
function emitMemoryUpdate(detail) {
  const snapshot = detail ?? {
    floating: floatingMemory.length,
    pinned: floatingMemory.filter((entry) => entry.pinned).length,
    ragEntries: ragStats.totalEntries,
    ragBytes: ragStats.totalBytes
  };
  document.dispatchEvent(new CustomEvent('memory:update', { detail: snapshot }));
}
...
async function loadAgentCache(agentId) {
  ...
  for (const entry of cache.entries) {
    entry.source = entry.source || 'cache';
    registerChunk(entry, 'cache');
  }
...
async function appendAgentCache(agentId, entry) {
  ...
  if (settings.memory.useEmbeddings) {
    try {
      ...
      if (vector) {
        await storeVectorCacheEntry(agentId, 'memory', record, vector);
        markChunkUsage(agentId, 'memory', record);
      }
    } catch (error) {
...
function bindMemoryHud() {
  document.addEventListener('memory:update', (event) => {
    const detail = event.detail || {};
    if (elements.floatingMemoryCount) {
      elements.floatingMemoryCount.textContent = String(detail.floating ?? 0);
    }
    if (elements.pinnedMemoryCount) {
      elements.pinnedMemoryCount.textContent = String(detail.pinned ?? 0);
    }
    if (elements.ragMemoryCount) {
      elements.ragMemoryCount.textContent = String(detail.ragEntries ?? 0);
    }
    if (elements.ragFootprint) {
      const sizeMb = ((detail.ragBytes ?? 0) / MB).toFixed(1);
      elements.ragFootprint.textContent = `${sizeMb} MB`;
    }
    if (elements.ragImportStatus) {
      const ragEntries = detail.ragEntries ?? 0;
      elements.ragImportStatus.textContent = ragEntries
        ? `Loaded ${ragEntries} cached embeddings.`
...
function populateReferences() {
  ...
  elements.vectorCacheEntries = document.getElementById('vectorCacheEntries');
  elements.vectorCacheSize = document.getElementById('vectorCacheSize');
  elements.vectorCacheHits = document.getElementById('vectorCacheHits');
  elements.vectorCacheMisses = document.getElementById('vectorCacheMisses');
...
function updateVectorCacheStatus() {
  if (!elements.vectorCacheStatus) return;
  if (!vectorCacheState.supported) {
    elements.vectorCacheStatus.textContent = 'Local Embedding Cache: Unavailable';
    if (elements.vectorCacheEntries) elements.vectorCacheEntries.textContent = '0';
    if (elements.vectorCacheSize) elements.vectorCacheSize.textContent = '0 MB';
    if (elements.vectorCacheHits) elements.vectorCacheHits.textContent = '0';
    if (elements.vectorCacheMisses) elements.vectorCacheMisses.textContent = '0';
    return;
  }
  const entries = Number(vectorCacheState.lastSize.entries) || 0;
  const sizeMb = (Number(vectorCacheState.lastSize.bytes || 0) / MB).toFixed(2);
  const readyText = vectorCacheState.ready
    ? `Local Embedding Cache: Ready (${entries} items, ${sizeMb} MB)`
    : `Local Embedding Cache: ${vectorCacheState.status || 'Warming…'}`;
  elements.vectorCacheStatus.textContent = readyText;
  if (elements.vectorCacheEntries) {
    elements.vectorCacheEntries.textContent = String(entries);
  }
  if (elements.vectorCacheSize) {
    elements.vectorCacheSize.textContent = `${sizeMb} MB`;
  }
  if (elements.vectorCacheHits) {
    elements.vectorCacheHits.textContent = String(vectorCacheState.hits);
  }
  if (elements.vectorCacheMisses) {
    elements.vectorCacheMisses.textContent = String(vectorCacheState.misses);
  }
}

function registerChunk(entry, source = 'memory') {
  if (!entry?.id) return;
  const existing = chunkRegistry.get(entry.id) || {};
  chunkRegistry.set(entry.id, { ...existing, ...entry, source });
}

function markChunkUsage(agentId, scope, entry) {
  if (!entry?.id) return;
  const key = `${agentId || 'global'}::${scope || 'memory'}::${entry.id}`;
  const current = chunkUsage.get(key) || { count: 0, entry, agentId, scope };
  current.count += 1;
  current.entry = entry;
  current.agentId = agentId;
  current.scope = scope;
  chunkUsage.set(key, current);
}

function makeCacheId(agentId, scope, entry) {
  return `${agentId || 'global'}::${scope || 'memory'}::${entry.id}`;
}

function buildCacheMeta(agentId, scope, entry) {
  return {
    ...
    turn: entry.turn ?? null,
    tokens: entry.tokens ?? estimateTokens(entry.content ?? ''),
    source: entry.source || scope || 'memory'
  };
}

function mergeRetrievalResults(cacheEntries = [], searchEntries = [], limit = Infinity) {
  const combined = [];
  const seen = new Set();
  const add = (entry) => {
  ...
  return limit < combined.length ? combined.slice(0, limit) : combined;
}

function scheduleVectorCacheSizeRefresh(delay = 300) {
  if (vectorCacheSizeTimer) return;
  vectorCacheSizeTimer = setTimeout(async () => {
    vectorCacheSizeTimer = null;
    if (!vectorCacheState.ready) return;
    try {
      const size = await VectorCache.size();
      vectorCacheState.lastSize = { entries: size.entries ?? 0, bytes: size.bytes ?? 0 };
      updateVectorCacheStatus();
    } catch (error) {
      console.warn('Vector cache size refresh failed', error);
    }
  }, delay);
}

function recordVectorCacheQuery(hits, topK) {
  vectorCacheState.hits += hits;
  vectorCacheState.misses += Math.max(0, topK - hits);
  vectorCacheState.missWindow.push({ hits, topK });
  if (vectorCacheState.missWindow.length > VECTOR_CACHE_WINDOW) {
    vectorCacheState.missWindow.shift();
  }
  updateVectorCacheStatus();
  maybePrimeVectorCache();
}

function computeMissRate() {
  if (!vectorCacheState.missWindow.length) return 0;
  let totalHits = 0;
  let totalTop = 0;
  for (const item of vectorCacheState.missWindow) {
    totalHits += item.hits;
    totalTop += item.topK;
  }
  if (totalTop === 0) return 0;
  return 1 - totalHits / totalTop;
}

function maybePrimeVectorCache() {
  if (!vectorCacheState.ready) return;
  if (vectorCacheState.primeRunning) return;
  const missRate = computeMissRate();
  if (missRate <= VECTOR_CACHE_MISS_THRESHOLD) {
    return;
  }
  const now = Date.now();
  if (now - vectorCacheState.lastPrime < VECTOR_CACHE_PRIME_COOLDOWN) {
    return;
  }
  vectorCacheState.primeRunning = true;
  vectorCacheState.lastPrime = now;
  void primeVectorCache().finally(() => {
    vectorCacheState.primeRunning = false;
  });
}

async function primeVectorCache() {
  const ranked = Array.from(chunkUsage.values())
    .sort((a, b) => b.count - a.count)
    .slice(0, VECTOR_CACHE_PRIME_LIMIT);
  for (const item of ranked) {
    if (!item.entry) continue;
    const targetAgents = item.agentId ? [item.agentId] : Object.keys(settings.agents || {});
    queueVectorCacheIngest(item.entry, targetAgents, item.scope || 'memory', true);
  }
}

async function queryVectorCache(agentId, scope, queryVector) {
  if (!vectorCacheState.ready || !queryVector) {
    return [];
  }
  try {
    const matches = await VectorCache.get(queryVector, {
      topK: VECTOR_CACHE_TOP_K,
      minSim: VECTOR_CACHE_MIN_SIM
    });
    const filtered = matches.filter((item) => {
      const meta = item.meta || {};
      if (meta.scope && meta.scope !== scope) return false;
      if (meta.agentId && agentId && meta.agentId !== agentId) return false;
      return true;
    });
    recordVectorCacheQuery(filtered.length, VECTOR_CACHE_TOP_K);
    const entries = filtered.map((item) => ({
      id: item.meta?.originId || item.id,
      content: item.meta?.content ?? '',
      ts: item.meta?.ts ?? null,
      round: item.meta?.round ?? null,
      turn: item.meta?.turn ?? null,
      agentId: item.meta?.agentId ?? agentId ?? null,
      tokens: item.meta?.tokens ?? estimateTokens(item.meta?.content ?? ''),
      source: item.meta?.source || scope,
      similarity: item.similarity ?? 0
    }));
    for (const entry of entries) {
      registerChunk(entry, entry.source || scope);
    }
    return entries;
  } catch (error) {
    console.warn('Vector cache query failed', error);
    recordVectorCacheQuery(0, VECTOR_CACHE_TOP_K);
    return [];
  }
}

async function storeVectorCacheEntry(agentId, scope, entry, embedding) {
  if (!vectorCacheState.ready || !embedding || !entry?.id) return;
  try {
    await VectorCache.putMany([
      {
        id: makeCacheId(agentId, scope, entry),
        embedding: embedding instanceof Float32Array ? embedding : Float32Array.from(embedding),
        meta: buildCacheMeta(agentId, scope, entry)
      }
    ]);
    scheduleVectorCacheSizeRefresh();
  } catch (error) {
    console.warn('Vector cache store failed', error);
  }
}

function queueVectorCacheIngest(entries, agentIds, scope, forceAll = false) {
  if (!vectorCacheState.supported) return;
  if (!settings.memory.useEmbeddings && !forceAll) return;
  const targets = Array.isArray(agentIds) && agentIds?.length ? agentIds : Object.keys(settings.agents || {});
  const payloads = Array.isArray(entries) ? entries : [entries];
  for (const entry of payloads) {
    if (!entry?.content) continue;
    const assignedScope = entry.scope || scope || 'memory';
    const agentList = forceAll && !targets.length ? Object.keys(settings.agents || {}) : targets;
    for (const agentId of agentList) {
      const agent = settings.agents?.[agentId];
      if (!agent?.embed?.model) continue;
      vectorCacheQueue.push({ agentId, entry, scope: assignedScope });
    }
  }
  scheduleVectorCacheIngest();
}

function scheduleVectorCacheIngest() {
  if (vectorCacheIngestTimer) return;
  const runner = async () => {
    vectorCacheIngestTimer = null;
    await flushVectorCacheQueue();
  };
  if (typeof requestIdleCallback === 'function') {
    vectorCacheIngestTimer = requestIdleCallback(() => {
      void runner();
    });
  } else {
    vectorCacheIngestTimer = setTimeout(() => {
      void runner();
    }, 0);
  }
}

async function flushVectorCacheQueue() {
  if (!vectorCacheState.supported || !settings.memory.useEmbeddings) {
    vectorCacheQueue.length = 0;
    return;
  }
  if (!vectorCacheState.ready) {
    return;
  }
  const slice = vectorCacheQueue.splice(0, VECTOR_CACHE_BATCH_SIZE);
  if (!slice.length) {
    return;
  }
  const grouped = new Map();
  for (const item of slice) {
    if (!item?.agentId || !item.entry?.content) continue;
    if (!grouped.has(item.agentId)) {
      grouped.set(item.agentId, []);
    }
    grouped.get(item.agentId).push(item);
  }
  for (const [agentId, items] of grouped.entries()) {
    const agent = settings.agents?.[agentId];
    if (!agent?.embed?.model) continue;
    const texts = items.map((item) => item.entry.content);
    let vectors = [];
    try {
      vectors = await callEmbedding(agent, texts);
    } catch (error) {
      console.warn('Vector cache ingest embedding failed', error);
      continue;
    }
    const records = [];
    for (let index = 0; index < items.length; index += 1) {
      ...
      const scope = items[index].scope || 'memory';
      records.push({
        id: makeCacheId(agentId, scope, entry),
    ...
    if (records.length) {
      await VectorCache.putMany(records);
    }
  }
  scheduleVectorCacheSizeRefresh();
  if (vectorCacheQueue.length) {
    scheduleVectorCacheIngest();
  }
}

async function setupVectorCache() {
  vectorCacheState.status = vectorCacheState.supported ? 'Warming…' : 'Unavailable';
  updateVectorCacheStatus();
  if (!vectorCacheState.supported) {
    return;
  }
  try {
    const size = await VectorCache.open({ dbName: 'sam-vectors', maxMB: 100 });
    if (size?.supported) {
      vectorCacheState.ready = true;
      vectorCacheState.lastSize = { entries: size.entries ?? 0, bytes: size.bytes ?? 0 };
      vectorCacheState.status = 'Ready';
      updateVectorCacheStatus();
      if (vectorCacheQueue.length) {
        scheduleVectorCacheIngest();
      }
    } else {
      vectorCacheState.ready = false;
      vectorCacheState.status = 'Unavailable';
      updateVectorCacheStatus();
    }
  } catch (error) {
    console.warn('Vector cache initialisation failed', error);
    vectorCacheState.ready = false;
    vectorCacheState.status = 'Error';
    updateVectorCacheStatus();
  }
}
...
async function loadBoard() {
  ...
  for (const entry of boardState.entries) {
    if (entry?.content) {
      registerChunk(entry, entry.summary ? 'summary' : 'board');
    }
  }
}
...
async function gatherBoardContext(agentId, prompt) {
  ...
  const cacheMatches = queryVector ? await queryVectorCache(agentId, 'board', queryVector) : [];
  for (const match of cacheMatches) {
    markChunkUsage(agentId, 'board', match);
  }
  const results = await vectorSearch({
    ...
    ensureEmbedding: async (entry) => {
      ...
      try {
        ...
        if (vector) {
          await storeVectorCacheEntry(agentId, 'board', entry, vector);
          markChunkUsage(agentId, 'board', entry);
        }
        return vector;
      } catch (error) {
  ...
  for (const item of results) {
    if (item?.entry) {
      markChunkUsage(agentId, 'board', item.entry);
    }
  }
  const merged = mergeRetrievalResults(cacheMatches, results, 3);
  return merged;
}
...
async function retrieveAgentMemories(agentId, prompt) {
  ...
  for (const candidate of candidates) {
    registerChunk(candidate, candidate.source || 'memory');
  }
  let queryVector = null;
  if (settings.memory.useEmbeddings) {
    try {
  ...
  const cacheMatches = queryVector ? await queryVectorCache(agentId, 'memory', queryVector) : [];
  for (const match of cacheMatches) {
    markChunkUsage(agentId, 'memory', match);
  }
  const results = await vectorSearch({
    candidates,
    limit,
    ...
    ensureEmbedding: async (entry) => {
      ...
      if (entry.source === 'cache') {
        ...
        try {
          ...
          await persistAgentCache(agentId);
          if (vector) {
            await storeVectorCacheEntry(agentId, 'memory', entry, vector);
            markChunkUsage(agentId, 'memory', entry);
          }
          return vector;
        } catch (error) {
  ...
  for (const item of results) {
    if (item?.entry) {
      markChunkUsage(agentId, 'memory', item.entry);
    }
  }
  const merged = mergeRetrievalResults(cacheMatches, results, limit);
  return merged;
}
...
async function maybeEmbed(agent, texts, options = {}) {
  ...
  try {
    const vectors = await callEmbedding(agent, texts);
...
function clearArenaCooldownTimer() {
  if (arenaState.cooldownTimer) {
    clearTimeout(arenaState.cooldownTimer);
    arenaState.cooldownTimer = null;
  }
}
...
async function maybeCooldown(id) {
  ...
  if (wait > 0) {
    ...
    await new Promise((resolve) => {
      arenaState.cooldownTimer = setTimeout(() => {
        arenaState.cooldownTimer = null;
        resolve();
      }, wait);
    });
